...
import { STUDIENFAECHER, FACH_BESCHREIBUNGEN, FRAGEN_BEREICHE, ANALYSE_BEREICHE, LERNGELEGENHEITEN } from './config.js';
        ...
        function mapAntworten() {
            ...
            ANALYSE_BEREICHE.forEach(bereich => {
                analyseWerte[bereich] = { subjektiv: [], objektiv: [], matrix: [], interesse: [] };
            });
                ...
                if (frage && frage.handlungsaspekte) {
                    frage.handlungsaspekte.forEach(aspekt => {
                        if (analyseWerte[aspekt]) {
                            // Matrix-Ebene ersetzt objektive Ebene für fachliche Bereiche
                            if (frage.ebene === 'matrix' || frage.ebene === 'objektiv') {
                                analyseWerte[aspekt].matrix.push(wert);
                            } else if (frage.ebene === 'subjektiv') {
                                analyseWerte[aspekt].subjektiv.push(wert);
                            } else if (frage.ebene === 'interesse') {
                                analyseWerte[aspekt].interesse.push(wert);
                            }
                        }
                    });
                }
        ...
        function drawRadar() {
            ...
            // Fachliche Kompetenzbereiche (haben Matrix-Daten)
            const fachlicheBereiche = [
                "Zahl und Variable",
                "Form und Raum",
            ...
            const chartData = ANALYSE_BEREICHE.map(bereich => {
                ...
                // Fachliche Bereiche nutzen Matrix-Werte statt objektiver Werte
                const objektiv = fachlicheBereiche.includes(bereich)
                    ? durchschnitt(bereichData.matrix.length > 0 ? bereichData.matrix : bereichData.objektiv)
                    : durchschnitt(bereichData.objektiv);
    ...
    if (currentPage < pages.length + 1) {
        ...
        function calculatePriorityPositions() {
            ...
            ANALYSE_BEREICHE.forEach(bereich => {
                ...
                let kompetenz, bedarf;

                // Unterschiedliche Berechnung für fachliche vs. didaktische Bereiche
                if (fachlicheBereiche.includes(bereich)) {
                    // Fachliche Bereiche: Nutze Matrix-Werte direkt (aus Standortbestimmung)
                    const matrixWerte = bereichData.matrix.length > 0 ? bereichData.matrix : bereichData.objektiv;
                    kompetenz = durchschnitt(matrixWerte);
                    bedarf = MAX_WERT - kompetenz;
                } else {
                    // Didaktische Bereiche & Handlungsaspekte: Nutze subjektive Selbsteinschätzungen
